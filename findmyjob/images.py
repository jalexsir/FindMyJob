...
FLAG_BAR_WIDTH = 8
# Відступ усіх трьох плашок (номер, NEW, обране) від країв картинки, до
# яких вони примикають, — однаковий з усіх боків: верх для номера й NEW,
# лівий бік для номера й обраного, правий для NEW, низ для обраного.
# Більше за INNER_PADDING навмисно — впритул до самого краю виглядає тісно
# (а Telegram ще й заокруглює кути бульбашки з фото на своєму боці).
...
class VacancyImageRenderer:
    ...
    def _draw_flag_stripe(image: Image.Image, width: int, height: int) -> None:
        ...
        лівого борту — від самого верху до самого низу, пропорційно
        навпіл (синій згори, жовтий знизу).

        Раніше малювалась через маску заокругленої рамки (щоб звужуватись
        на кутах під колишню жовту обвідку) — та обвідку прибрано
        (`render()` більше не малює `outline`), і без неї заокруглення лише
        вкорочувало стрічку на кілька пікселів згори/знизу замість "на всю
        висоту". Тепер — звичайні прямокутники, без маски.
        """
        draw = ImageDraw.Draw(image)
        half = height // 2
        draw.rectangle([0, 0, FLAG_BAR_WIDTH - 1, half - 1], fill=FLAG_BLUE)
        draw.rectangle([0, half, FLAG_BAR_WIDTH - 1, height - 1], fill=YELLOW)
