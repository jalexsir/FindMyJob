...
GENERIC_LEADING_WORDS = {
    ...
    # "ми" (укр.) бракувало поряд з англ. "we" — "Ми розробляємо…",
    # "Ми створюємо…" мали пройти повз is_invalid_company саме через це.
    "ми",
    # Заголовки/уривки опису, що трапляються як окремі "кандидати в компанію":
    # "Про проєкт", "Плюсом буде: …", "Офіс", "Основний фокус ролі",
    # "Зараз команда…", "Ідеальний кандидат…".
    "про", "плюсом", "офіс", "основний", "зараз", "ідеальний", "кандидат",
...
SECTION_HEADERS = {
    ...
    "завдання", "головне завдання", "ключові завдання",
...
# "запрошує" навмисно відсутнє: "<Ім'я> запрошує на співбесіду" — типова
# фраза від рекрутера-людини, а не компанії ("Вітя запрошує…" ловилось
# як назва компанії "Вітя").
_ACTION_VERBS = (
    "розробляє", "займається", "шукає",
...
# "Команда з ~4 осіб" — дієслово-маркер ("...осіб працює") захоплює "4 осіб"
# як кандидата (тильда не входить у дозволені символи регексу, тож збіг
# стартує вже з цифри) — розмір команди, а не назва. _YEARS_RE це не ловить:
# він лише про "5 років"/"3 years", не про кількість людей.
_TEAM_SIZE_RE = re.compile(
    r"^\d+[\+]?\s*(?:осіб|особи|особа|людей|людини|чоловік|фахівців|"
    r"розробників|співробітників|учасників)"
)
# Пункт нумерованого списку ("1. Venture fundraising") — на відміну від
# "414 окрема бригада" (цифри без крапки/дужки одразу після), тут за цифрою
# йде "." чи ")" — типовий маркер списку завдань/вимог з опису вакансії.
...
def is_invalid_company(text: str, title: str = "") -> bool:
    ...
    if _YEARS_RE.match(candidate.lower()):
        return True
    # Розмір команди ("4 осіб", "10 людей") — теж не назва
    if _TEAM_SIZE_RE.match(candidate.lower()):
        return True
    # Пункт нумерованого списку ("1. Venture fundraising") — не назва
    if _LIST_ITEM_RE.match(candidate):
        return True
    # Незбалансовані дужки ("React)") — обрізок фрагмента опису, а не назва
