...
# із кількістю категорій: для всіх 55 це 138 потоків і стільки ж одночасних
# з'єднань до двох хостів — на 1 OCPU і зайва пам'ять, і привід для сайтів
# відповісти 429.
#
# Саме 12, а не менше: максимум для ручного пошуку — 6 категорій
# (MAX_SELECTED_CATEGORIES), тобто 24 джерела, і при 12 воркерах це рівно дві
# хвилі (з попереднім лімітом у 5 категорій/20 джерел виміряно: 1.2 с проти
# 0.6 с без стелі, тоді як при 8 було б 1.8 с — форма "дві хвилі" з підняттям
# ліміту не зламалась, 24 теж ділиться на 12 без остачі). Джоба зі всіма
# категоріями відпрацює за ~7 с замість ~1 с, але вона годинна, і там це не
# має значення.
